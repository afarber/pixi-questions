...
test.describe('WebSocket Reconnection Tests', () => {
  test('automatically reconnects after connection loss', async ({ page }) => {
    ...
    await page.locator('#messageInput').fill('Before disconnect');
    await page.locator('#sendButton').click();
    await expect(page.locator('#chatWindow')).toContainText('Before disconnect');
    
    // Simulate network disconnection by blocking WebSocket requests
    await page.route('**/ws', route => route.abort());
    
    // Trigger a message send that will fail
    await page.locator('#messageInput').fill('This should fail');
    await page.locator('#sendButton').click();
    
    // Should show disconnected status
    await expect(page.locator('#connectionStatus')).toHaveText('Disconnected', { timeout: 10000 });
    await expect(page.locator('#sendButton')).toBeDisabled();
    
    // Should show reconnecting status
    await expect(page.locator('#connectionStatus')).toHaveText('Reconnecting...', { timeout: 15000 });
    ...
    await page.locator('#messageInput').fill('After reconnect');
    await page.locator('#sendButton').click();
    await expect(page.locator('#chatWindow')).toContainText('After reconnect');
  });
  ...
  test('maintains user state during reconnection', async ({ page }) => {
    ...
    await page.locator('#messageInput').fill('Reconnected automatically');
    await page.locator('#sendButton').click();
    await expect(page.locator('#chatWindow')).toContainText('Reconnected automatically');
  });
  ...
  test('handles multiple quick disconnections gracefully', async ({ page }) => {
    ...
    await page.locator('#messageInput').fill('Survived multiple disconnects');
    await page.locator('#sendButton').click();
    await expect(page.locator('#chatWindow')).toContainText('Survived multiple disconnects');
  });
  ...
  test('shows proper connection status indicators', async ({ page }) => {
    ...
    const connectedStyles = await page.locator('#connectionStatus').evaluate(el => {
    ...
    const disconnectedStyles = await page.locator('#connectionStatus').evaluate(el => {
      return {
        className: el.className
      };
    });
    ...
    const reconnectingStyles = await page.locator('#connectionStatus').evaluate(el => {
      return {
        className: el.className
      };
    });
  ...
  test('handles server restart gracefully', async ({ page }) => {
    ...
    await page.locator('#messageInput').fill('Before server restart');
    await page.locator('#sendButton').click();
    await expect(page.locator('#chatWindow')).toContainText('Before server restart');
    ...
    await page.locator('#messageInput').fill('After server restart');
    await page.locator('#sendButton').click();
    await expect(page.locator('#chatWindow')).toContainText('After server restart');
  });
  ...
  test('reconnection works across different network conditions', async ({ page }) => {
    ...
    await page.locator('#messageInput').fill('Slow network test');
    await page.locator('#sendButton').click();
    ...
    await page.locator('#messageInput').fill('Network conditions handled');
    await page.locator('#sendButton').click();
    await expect(page.locator('#chatWindow')).toContainText('Network conditions handled');
  });
});